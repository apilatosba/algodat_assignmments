...
\begin{itemize}
  \item \textbf{Base Case:} For height \( h = 0 \), the minimum number of nodes in an AVL tree is \( F_{0+2} - 1 = F_2 - 1 = 1 - 1 = 0 \), which is correct since an AVL tree of height 0 has no nodes.
  \item \textbf{Inductive Step:} Assume that for some height \( h \geq 0 \), the minimum number of nodes in an AVL tree of height \( h \) is \( F_{h+2} - 1 \). We need to show that this holds for height \( h + 1 \).
  \item Consider an AVL tree of height \( h + 1 \). To minimize the number of nodes, we can have one subtree of height \( h \) and another subtree of height \( h - 1 \) (since the height difference must be at most 1 for AVL trees).
  \item By the inductive hypothesis the minimum number of nodes in the subtree of height \( h \) is \( F_{h+2} - 1 \) and in the subtree of height \( h - 1 \) is \( F_{(h-1)+2} - 1 = F_{h+1} - 1 \).
  \item Therefore the total minimum number of nodes in the AVL tree of height \( h + 1 \) is:
    \[N(h + 1) = (F_{h+2} - 1) + (F_{h+1} - 1) + 1 = F_{h+2} + F_{h+1} - 1 = F_{(h+1)+2} - 1\]
    (using the Fibonacci property \( F_{n} = F_{n-1} + F_{n-2} \)).
  \item Thus by induction the statement holds for all heights \( h \geq 0 \).
\end{itemize}

\section*{Exercise 3}
\subsection*{(a)}
To implement the Min(h) operation, we can follow the left child pointers starting from the node
represented by handle h until we reach a node that does not have a left child. This node will be the
node with the smallest key in the subtree rooted at h since AVL trees are just BSTs with a constraint on the height of the nodes. The algorithm is as follows:
\begin{verbatim}
Handle Min(Handle h)
    current := h
...
\begin{itemize}
  \item \textbf{Insert:} When inserting a new node, we start from the root and traverse down to find the correct position for the new node. As we traverse back up the tree, we increment the size attribute of each node by 1 to account for the newly added node.
    And when we do Rotate operations updating the size attribute of the nodes affected is just constant time because there is only three nodes affected by that in the Rotate operations.
  \item \textbf{Remove:} When removing a node, we first find the node to be removed. After removing the node, we traverse back up the tree and decrement the size attribute of each node by 1 to account for the removed node.
    And similar to Insert operation, when we do Rotate operations updating the size attribute of the nodes affected is just constant time because there is only three nodes affected by that in the Rotate operations.
\end{itemize}
...
\subsection*{(c)}
To implement the Rank(k) operation, we can use the size attribute of each node to count the number of nodes with keys less than or equal to k. The algorithm is as follows:
\begin{verbatim}
int Rank(Key k)
    current := root
    result := 0
